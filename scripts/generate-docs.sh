...
# This script update swagger.yml from proto of finschia-sdk. And also update statik file.
#
# requirements
...
# Get the path of the finschia-sdk repo from go/pkg/mod
...
if [ -d "${finschia_sdk_dir}/proto" ]; then
  proto_dirs=$(find "${finschia_sdk_dir}/proto" -name '*.proto' -print0 | xargs -0 -n1 dirname | sort | uniq)
