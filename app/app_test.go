...
func TestSimAppExportAndBlockedAddrs(t *testing.T) {
	...
	app := NewFnsaAppWithCustomOptions(t, false, SetupOptions{
	...
	app2 := NewFnsaApp(logger.With("instance", "second"), db, nil, true, simtestutil.NewAppOptionsWithFlagHome(t.TempDir()))
...
func TestRunMigrations(t *testing.T) {
	db := dbm.NewMemDB()
	logger := log.NewTestLogger(t)
	app := NewFnsaApp(logger.With("instance", "simapp"), db, nil, true, simtestutil.NewAppOptionsWithFlagHome(t.TempDir()))
...
func TestInitGenesisOnMigration(t *testing.T) {
	db := dbm.NewMemDB()
	app := NewFnsaApp(log.NewTestLogger(t), db, nil, true, simtestutil.NewAppOptionsWithFlagHome(t.TempDir()))
