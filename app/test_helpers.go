...
// SetupOptions defines arguments that are passed into `FnsaApp` constructor.
...
func setup(t testing.TB, chainID string, withGenesis bool, invCheckPeriod uint, opts ...wasmkeeper.Option) (*FnsaApp, GenesisState) {
	db := dbm.NewMemDB()
	nodeHome := t.TempDir()
	...
	app := NewFnsaApp(log.NewNopLogger(), db, nil, true, appOptions, opts, bam.SetChainID(chainID), bam.SetSnapshot(snapshotStore, snapshottypes.SnapshotOptions{KeepRecent: 2}))
	if withGenesis {
...
// NewFnsaAppWithCustomOptions initializes a new FnsaApp with custom options.
func NewFnsaAppWithCustomOptions(t *testing.T, isCheckTx bool, options SetupOptions) *FnsaApp {
	...
	app := NewFnsaApp(options.Logger, options.DB, nil, true, options.AppOpts, options.WasmOpts)
...
func Setup(t *testing.T, opts ...wasmkeeper.Option) *FnsaApp {
	t.Helper()
...
func SetupWithEmptyStore(t testing.TB) *FnsaApp {
...
func NewTestNetworkFixture() network.TestFixture {
	...
	app := NewFnsaApp(log.NewNopLogger(), dbm.NewMemDB(), nil, true, simtestutil.NewAppOptionsWithFlagHome(dir), emptyWasmOptions)
